import { AlertPolicy } from '@shared/enums/alert-policy.enum';
import { DatasetRecordSize, DatasetType } from '@shared/enums/dataset.enum';
import { DeviceType } from '@shared/enums/device-type.enum';
import { EnclosureSlotStatus } from '@shared/enums/enclosure-slot-status.enum';
...
import { OnOff } from '@shared/enums/on-off.enum';
import { ProductType } from '@shared/enums/product-type.enum';
import { ServiceName } from '@shared/enums/service-name.enum';
import { SmbInfoLevel } from '@shared/enums/smb-info-level.enum';
import { SystemEnvironment } from '@shared/enums/system-environment.enum';
import { TransportMode } from '@shared/enums/transport-mode.enum';
import {
  Acl,
  AclQueryParams,
  AclTemplateByPath,
...
} from '@shared/interfaces/acl.interface';
import { ActiveDirectoryConfig } from '@shared/interfaces/active-directory-config.interface';
import { AdvancedConfig, AdvancedConfigUpdate } from '@shared/interfaces/advanced-config.interface';
import { AlertService, AlertServiceEdit } from '@shared/interfaces/alert-service.interface';
import {
...
import { ApiKey, CreateApiKeyRequest, UpdateApiKeyRequest } from '@shared/interfaces/api-key.interface';
import { UpgradeSummary } from '@shared/interfaces/application.interface';
import { AuditConfig, AuditEntry, AuditQueryParams } from '@shared/interfaces/audit/audit.interface';
import { AuthSession } from '@shared/interfaces/auth-session.interface';
import { LoginQuery } from '@shared/interfaces/auth.interface';
import { AvailableApp } from '@shared/interfaces/available-app.interface';
import {
  Bootenv,
  CreateBootenvParams,
  SetBootenvAttributeParams,
  UpdateBootenvParams,
} from '@shared/interfaces/bootenv.interface';
import {
  Catalog, CatalogApp,
  CatalogItems,
...
} from '@shared/interfaces/certificate-authority.interface';
import {
  Certificate,
  CertificateProfiles,
...
import {
  ChartReleaseEvent,
} from '@shared/interfaces/chart-release-event.interface';
import {
  ChartRelease,
  ChartReleaseQueryParams,
  ChartReleaseUpgradeParams,
} from '@shared/interfaces/chart-release.interface';
import { Choices } from '@shared/interfaces/choices.interface';
import { CloudBackup, CloudBackupSnapshot, CloudBackupUpdate } from '@shared/interfaces/cloud-backup.interface';
import {
  CloudSyncDirectoryListing,
  CloudSyncListDirectoryParams,
  CloudSyncTask,
  CloudSyncTaskUpdate,
} from '@shared/interfaces/cloud-sync-task.interface';
import {
  CloudSyncBucket,
  CloudSyncCredential,
  CloudSyncCredentialUpdate,
  CloudSyncCredentialVerify, CloudSyncCredentialVerifyResult,
} from '@shared/interfaces/cloudsync-credential.interface';
import { CloudSyncProvider, CloudSyncRestoreParams } from '@shared/interfaces/cloudsync-provider.interface';
import { ContainerConfig, ContainerConfigUpdate } from '@shared/interfaces/container-config.interface';
import {
  ContainerImage, DeleteContainerImageParams,
} from '@shared/interfaces/container-image.interface';
import { CoreDownloadQuery, CoreDownloadResponse } from '@shared/interfaces/core-download.interface';
import {
  CountManualSnapshotsParams,
  EligibleManualSnapshotsCount,
  TargetUnmatchedSnapshotsParams,
} from '@shared/interfaces/count-manual-snapshots.interface';
import { Cronjob, CronjobUpdate } from '@shared/interfaces/cronjob.interface';
import { DatasetHasVmsQueryParams } from '@shared/interfaces/dataset-has-vms-query-params.interface';
import { DatasetQuota, DatasetQuotaQueryParams, SetDatasetQuota } from '@shared/interfaces/dataset-quota.interface';
import {
...
} from '@shared/interfaces/dns-authenticator.interface';
import { DockerHubRateLimit } from '@shared/interfaces/dockerhub-rate-limit.interface';
import {
  DsUncachedGroup, DsUncachedUser, LoggedInUser,
} from '@shared/interfaces/ds-cache.interface';
import { EnclosureUi } from '@shared/interfaces/enclosure.interface';
import {
  FailoverConfig,
  FailoverUpdate,
} from '@shared/interfaces/failover.interface';
import { FileRecord, ListdirQueryParams } from '@shared/interfaces/file-record.interface';
import { FileSystemStat, Statfs } from '@shared/interfaces/filesystem-stat.interface';
import { FtpConfig, FtpConfigUpdate } from '@shared/interfaces/ftp-config.interface';
import {
  CreateGroup, DeleteGroupParams, Group, UpdateGroup,
} from '@shared/interfaces/group.interface';
import { IdmapBackendOptions } from '@shared/interfaces/idmap-backend-options.interface';
import { Idmap, IdmapUpdate } from '@shared/interfaces/idmap.interface';
import {
  CreateInitShutdownScript,
  InitShutdownScript,
  UpdateInitShutdownScriptParams,
} from '@shared/interfaces/init-shutdown-script.interface';
import {
  Ipmi, IpmiChassis, IpmiQueryParams, IpmiUpdate,
} from '@shared/interfaces/ipmi.interface';
import {
  IscsiGlobalConfig,
  IscsiGlobalConfigUpdate,
  IscsiGlobalSession,
} from '@shared/interfaces/iscsi-global-config.interface';
import {
...
} from '@shared/interfaces/iscsi.interface';
import { Jbof, JbofUpdate } from '@shared/interfaces/jbof.interface';
...
} from '@shared/interfaces/kerberos-config.interface';
import { KerberosRealm, KerberosRealmUpdate } from '@shared/interfaces/kerberos-realm.interface';
import {
  KeychainCredential,
  KeychainCredentialCreate, KeychainCredentialUpdate,
  KeychainSshCredentials,
  SshKeyPair,
} from '@shared/interfaces/keychain-credential.interface';
import { KmipConfig } from '@shared/interfaces/kmip-config.interface';
import { KubernetesConfig } from '@shared/interfaces/kubernetes-config.interface';
import { KubernetesStatusData } from '@shared/interfaces/kubernetes-status-data.interface';
import { LdapConfig } from '@shared/interfaces/ldap-config.interface';
import { MailConfig, MailConfigUpdate } from '@shared/interfaces/mail-config.interface';
import {
  NetworkConfiguration,
  NetworkConfigurationUpdate,
} from '@shared/interfaces/network-configuration.interface';
import {
  NetworkInterface,
  NetworkInterfaceCreate, NetworkInterfaceUpdate,
  ServiceRestartedOnNetworkSync,
} from '@shared/interfaces/network-interface.interface';
import { NetworkSummary } from '@shared/interfaces/network-summary.interface';
import { AddNfsPrincipal, NfsConfig, NfsConfigUpdate } from '@shared/interfaces/nfs-config.interface';
import {
  Nfs3Session, Nfs4Session, NfsShare, NfsShareUpdate,
} from '@shared/interfaces/nfs-share.interface';
import { CreateNtpServer, NtpServer } from '@shared/interfaces/ntp-server.interface';
import { MapOption } from '@shared/interfaces/option.interface';
import {
  PeriodicSnapshotTask,
  PeriodicSnapshotTaskCreate,
  PeriodicSnapshotTaskUpdate,
} from '@shared/interfaces/periodic-snapshot-task.interface';
import { DatasetAttachment, PoolAttachment } from '@shared/interfaces/pool-attachment.interface';
import { CreatePoolScrubTask, PoolScrubTask } from '@shared/interfaces/pool-scrub.interface';
import {
  Pool, PoolInstance,
} from '@shared/interfaces/pool.interface';
import { Privilege, PrivilegeRole, PrivilegeUpdate } from '@shared/interfaces/privilege.interface';
import { Process } from '@shared/interfaces/process.interface';
import { QueryParams } from '@shared/interfaces/query-api.interface';
import { ReplicationConfigUpdate } from '@shared/interfaces/replication-config-update.interface';
import { ReplicationConfig } from '@shared/interfaces/replication-config.interface';
import {
  ReplicationCreate,
  ReplicationTask,
} from '@shared/interfaces/replication-task.interface';
import {
  CreateReportingExporter, ReportingExporter, ReportingExporterSchema, UpdateReportingExporter,
} from '@shared/interfaces/reporting-exporters.interface';
import { ReportingGraph } from '@shared/interfaces/reporting-graph.interface';
import {
  ReportingData,
  ReportingQueryParams,
} from '@shared/interfaces/reporting.interface';
import { ResilverConfig, ResilverConfigUpdate } from '@shared/interfaces/resilver-config.interface';
import { RsyncTask, RsyncTaskUpdate } from '@shared/interfaces/rsync-task.interface';
import { Service } from '@shared/interfaces/service.interface';
import { ResizeShellRequest } from '@shared/interfaces/shell.interface';
import {
  SmartManualTestParams, SmartConfig, SmartConfigUpdate, SmartTestTask, SmartTestResults, ManualSmartTest,
  SmartTestTaskUpdate,
} from '@shared/interfaces/smart-test.interface';
import { SmbConfig, SmbConfigUpdate } from '@shared/interfaces/smb-config.interface';
import {
  SmbPresets, SmbShare, SmbSharesec, SmbSharesecAce, SmbShareUpdate,
} from '@shared/interfaces/smb-share.interface';
import { SmbStatus } from '@shared/interfaces/smb-status.interface';
import { SnmpConfig, SnmpConfigUpdate } from '@shared/interfaces/snmp-config.interface';
import { SshConfig, SshConfigUpdate } from '@shared/interfaces/ssh-config.interface';
import {
  RemoteSshScanParams,
  SshConnectionSetup,
  SshSemiAutomaticSetup,
} from '@shared/interfaces/ssh-connection-setup.interface';
import { StaticRoute, UpdateStaticRoute } from '@shared/interfaces/static-route.interface';
import {
...
import { SystemGeneralConfig, SystemGeneralConfigUpdate } from '@shared/interfaces/system-config.interface';
import { SystemDatasetConfig } from '@shared/interfaces/system-dataset-config.interface';
import { SystemInfo } from '@shared/interfaces/system-info.interface';
import { SystemSecurityConfig } from '@shared/interfaces/system-security-config.interface';
import {
  SystemUpdate,
  SystemUpdateChange,
  SystemUpdateTrains,
} from '@shared/interfaces/system-update.interface';
import {
  TrueCommandConfig,
  TrueCommandConnectionState, TrueCommandUpdateResponse,
  UpdateTrueCommand,
} from '@shared/interfaces/true-command-config.interface';
import { Tunable } from '@shared/interfaces/tunable.interface';
import { GlobalTwoFactorConfig, GlobalTwoFactorConfigUpdate, UserTwoFactorConfig } from '@shared/interfaces/two-factor-config.interface';
import { UpsConfig, UpsConfigUpdate } from '@shared/interfaces/ups-config.interface';
import {
