...
describe('ParamsBuilder – filters groups', () => {
    describe('group()', () => {
        it('replaces current filters with ones from group() when it is called', () => {
            const builder = new ParamsBuilder<User>().group((group) => {
                group.filter('username', '=', 'bob').andFilter('username', '=', 'admin');
            });
        ...
        it('condition after the group', () => {
            const builder = new ParamsBuilder<User>()
                .group((group) => {
                    group.filter('username', '=', 'bob').orFilter('username', '=', 'admin');
                })
                .andFilter('age', '>', 18);
    ...
    describe('andGroup()', () => {
        it('after filter()', () => {
            const builder = new ParamsBuilder<User>().filter('age', '>', 45).andGroup((group) => {
                group.filter('username', '=', 'adam').orFilter('username', '=', 'eve');
            });
    ...
    describe('orGroup()', () => {
        it('after filter()', () => {
            const builder = new ParamsBuilder<User>().filter('age', '>', 45).orGroup((group) => {
                group.filter('username', '=', 'adam').andFilter('age', '>', 30);
            });
    ...
    describe('mixed groups', () => {
        it('supports nested groups', () => {
            const builder = new ParamsBuilder<User>()
                ...
                .orFilter('age', '=', 46)
                .orGroup((group) => {
                    group.filter('username', '=', 'adam').andGroup((subgroup) => {
                        subgroup.filter('age', '=', 30).orFilter('age', '=', 20);
                    });
                });
        ...
        it('two groups via OR - first group is OR', () => {
            const builder = new ParamsBuilder<User>()
                ...
                .orGroup((group) => {
                    group.filter('age', '<', 45).andFilter('age', '>', 18);
                });
        ...
        it('two groups via OR - first group is AND', () => {
            const builder = new ParamsBuilder<User>()
                .group((group) => {
                    group.filter('username', '=', 'bob').andFilter('locked', '=', true);
                })
                .orGroup((group) => {
                    group.filter('age', '<', 45).andFilter('age', '>', 18);
                });
